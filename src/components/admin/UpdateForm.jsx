...
export function UpdateForm ({ showModalUpdate, handleModalUpdate }) {
  ...
  const handleSubmitForm = async (e) => {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL_LOCAL}products/${showModalUpdate.item.id}`,{
      method: 'PATCH',
      credentials: 'include',
      body: data
    })
  ...
  return (
    <section role='modal' className={`${showModalUpdate.show ? 'flex' : 'hidden'} filter backdrop-blur-sm   absolute  top-0 z-1  w-full h-full justify-center items-center`}>
      ...
      <form onSubmit={handleSubmitForm} className='bg-[#FD70A7] relative rounded-2xl gap-2 shadow-2xl flex flex-col justify-center items-center py-5 w-[500px]'  encType="multipart/form-data">
